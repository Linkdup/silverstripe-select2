...
class AjaxSelect2Field extends TextField
{
    ...
    public function Field($properties = [])
    {
        ...
        Requirements::javascript(
            'sheadawson/silverstripe-select2: select2/select2.js'
        );
        Requirements::javascript(
            'sheadawson/silverstripe-select2: javascript/ajaxselect2.init.js'
        );
        Requirements::css(
            'sheadawson/silverstripe-select2: select2/select2.min.css'
        );
    ...
    public function search($request)
    {
        ...
        foreach ($searchFields as $searchField) {
            $name =
                strpos($searchField, ':') !== false
                    ? $searchField
                    : "$searchField:PartialMatch";
            $params[$name] = $request->getVar('term');
        }
        $start = (int) $request->getVar('id')
            ? (int) $request->getVar('id') * $this->getConfig('resultsLimit')
            : 0;

        $list = $list
            ->filterAny($params)
            ->exclude($this->getConfig('excludes'));
        ...
        $results = $list
            ...
            ->limit($this->getConfig('resultsLimit'), $start);
        ...
        $originalSourceFileComments = Config::inst()->get(
            'SSViewer',
        ...
        foreach ($results as $object) {
            $return['list'][] = [
                'id' => $object->ID,
                'resultsContent' => html_entity_decode(
                    SSViewer::fromString(
                        $this->getConfig('resultsFormat')
                    )->process($object)
                ),
                'selectionContent' => SSViewer::fromString(
                    $this->getConfig('selectionFormat')
                )->process($object),
            ];
        }
        // Config::inst()->set(
    ...
    public function getAttributes()
    {
        ...
        if (
            $this->Value() &&
            ($object = DataObject::get($this->getConfig('classToSearch'))->byID(
                $this->Value()
            ))
        ) {
            $originalSourceFileComments = Config::inst()->get(
            ...
            $attributes['data-selectioncontent'] = html_entity_decode(
                SSViewer::fromString(
                    $this->getConfig('selectionFormat')
                )->process($object)
            );
